...
As "find" also exists on Dos/Windows, but with a different syntax, the linux port must appear in the Windows system PATH before the Windows one:
With msys2 distribution, C:\msys64\usr\bin must be declared before C:\Windows\system32

wxrc can be built as a wxWidgets tool (but build it as static because dynamic version does not seem to work as expected).
...
If necessary, put the content of Unix_Tools somewhere in your PATH: if you have msys2, they should already exist in C:\msys64\usr\bin except wxrc.exe, but this executable must also be accessible via the system PATH.